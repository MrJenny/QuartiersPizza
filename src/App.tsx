...
import { Calendar, Clock, User, Plus, Trash2, Pizza, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Reservatione hend nöd chöne glade werde.');
      const data = await response.json();
      setBookings(data);
    } catch (err) {
      setError('Reservatione hend nöd chöne glade werde. Bitte probier\'s spöter nomal.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#f5f5f0] text-stone-900">
      <header className="relative h-[40vh] flex items-center justify-center overflow-hidden bg-[#5A5A40] text-white">
        ...
        <div className="relative z-10 text-center px-4">
          <h1 className="text-5xl md:text-7xl font-serif font-light tracking-tight mb-2">Quatiers-Pizza-Ofe</h1>
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          <div className="flex items-center gap-4">
            <button onClick={() => changeDate(-1)} className="p-2 hover:bg-stone-200 rounded-full transition-colors">
              <ChevronLeft size={24} />
            </button>
            <div className="text-center min-w-[240px]">
              <h2 className="text-3xl font-serif font-medium">{formatDate(selectedDate)}</h2>
          ...
          <button 
            ...
            className="bg-[#5A5A40] text-white px-8 py-4 rounded-full font-medium shadow-xl hover:opacity-90 transition-all"
          >
            + Plätzli reserviere
          </button>
        </div>
        ...
        <div className="grid gap-6">
          {loading ? (
            ...
            getBookingsForDate(selectedDate).map((booking) => (
              <div key={booking.id} className="bg-white p-6 rounded-3xl shadow-sm border border-stone-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    <h3 className="text-2xl font-serif font-medium">{booking.name}</h3>
            ...
            <div className="text-center py-20 bg-stone-50 rounded-3xl border-2 border-dashed border-stone-200">
              <h3 className="text-xl font-serif text-stone-400">No kei Reservatione für hüt.</h3>
            </div>
          )}
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[2rem] shadow-2xl overflow-hidden">
            <div className="bg-[#5A5A40] p-8 text-white">
              <h2 className="text-3xl font-serif">Ofe reserviere</h2>
            </div>
            <form onSubmit={handleBooking} className="p-8 space-y-6">
              <div>
                ...
                <input 
                  required 
                  type="text" 
                  className="w-full px-4 py-4 bg-stone-50 border-none rounded-2xl focus:ring-2 focus:ring-[#5A5A40]" 
                  value={formData.name} 
              ...
              <div>
                ...
                <textarea 
                  className="w-full p-4 bg-stone-50 border-none rounded-2xl min-h-[100px] focus:ring-2 focus:ring-[#5A5A40]" 
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className="flex-1 py-4 border border-stone-200 rounded-2xl"
                >
                  Abbreche
                </button>
                <button 
                  type="submit" 
                  className="flex-1 py-4 bg-[#b35a38] text-white rounded-2xl shadow-lg"
                >
                  Bestätige
                </button>
              </div>
